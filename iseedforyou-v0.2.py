...
if use_key1:
    ...
    entropia = binascii.hexlify(key1).decode()
    ...
    entropia = binascii.hexlify(key2).decode()
...
if opcao_selecionada == "SEED 12 Palavras":
    command = f'echo {entropia} | python3 rolls12.py'
    result = subprocess.run(command, shell=True, text=True, capture_output=True)
    print(result.stdout)

elif opcao_selecionada == "SEED 24 Palavras":
    command = f'echo {entropia} | python3 rolls.py'
    result = subprocess.run(command, shell=True, text=True, capture_output=True)
    print(result.stdout)
